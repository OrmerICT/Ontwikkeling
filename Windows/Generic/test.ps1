...
function Format-DirectoryTreeAsXML($XMLElement, $Foldertree)
{
    if($foldertree.Files.Count -gt 0)
    {
        if ($foldertree.Files.Count -gt 1)
        {
            $files = $foldertree.Files | Sort-Object -Descending {$_.SizeBytes}
        }
        else
        {
        ...
        for ($i = 0; $i -lt $foldertree.Files.Count; $i++)
        {
    ...
    if($foldertree.Folders.Count -gt 0)
    {
        if($foldertree.Folders.Count -gt 1)
        {
            $folders = $foldertree.Folders | Sort-Object -Descending {$_.SizeBytes}
        }
        else
        {
        ...
        for ($i = 0; $i -lt $foldertree.Folders.Count; $i++)
        {             
            ...
            Format-DirectoryTreeAsXML -XMLElement $neldir -Foldertree $folders[$i]            
        }
    }    
}
$directoryTree = @{}
Get-DirectoryTree -DirectoryTreeObject $directoryTree -CurrentDirInfo $PathToProcess
[xml]$xml = New-Object -TypeName System.Xml.XmlDocument
$xmlroot = "root"
$xml.AppendChild($xml.CreateXmlDeclaration('1.0', 'utf-8', $null)) | Out-Null
$xml.AppendChild( ($rootelement = $xml.CreateElement($xmlroot))) | Out-Null
Format-DirectoryTreeAsXML -XMLElement $rootelement -Foldertree $directoryTree
$xml.Save("$($env:USERPROFILE)\Desktop\Output.XML")